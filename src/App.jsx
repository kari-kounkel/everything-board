...
const EMP_PLATFORMS = ["Facebook", "Instagram", "LinkedIn", "Substack", "YouTube"];
const EMP_PLATFORM_COLORS = {
...
const EMP_DAYS = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"];

function EmpireCommandCenter({ userId }) {
  const { apps, saveApp, removeApp } = useEmpireApps(userId);
  const { subs, saveSub, removeSub } = useEmpireSubs(userId);
  const { posts, savePost, removePost } = useEmpirePosts(userId);
  ...
  const [newApp, setNewApp] = useState({ name: "", url: "", stack: "", status: "planning", universe: "", description: "", color: "#c9a84c" });
  ...
  const UNIVERSES_EMP = ["All", ...Array.from(new Set(apps.map(a => a.universe).filter(Boolean)))];
  const CATEGORIES_EMP = ["All", ...Array.from(new Set(subs.map(s => s.category).filter(Boolean)))];

  const monthlyCost = subs.filter(s => s.status === "active" && s.billing === "monthly").reduce((sum, s) => sum + (Number(s.cost) || 0), 0);
  ...
  const inputS = { width: "100%", padding: "8px 10px", borderRadius: "8px", border: "1.5px solid #E0D8C8", background: "#FAF8F3", fontSize: "13px", fontFamily: "inherit", outline: "none", marginTop: "4px" };
  const labelS = { fontSize: "10px", fontWeight: 700, color: "#999", textTransform: "uppercase", letterSpacing: "1px", fontFamily: "inherit" };

  return (
    <div style={{ minHeight: "100vh", background: "#F5F2EA", fontFamily: "'DM Sans', sans-serif", padding: "0" }}>
      ...
      {/* Header */}
      <div style={{ background: "#1A1208", padding: "16px 24px" }}>
        <div style={{ fontFamily: "'Playfair Display', serif", fontSize: "1.3rem", fontWeight: 900, color: "#E8B931", marginBottom: "4px" }}>Empire Command Center</div>
        <div style={{ display: "flex", gap: "10px", flexWrap: "wrap", marginTop: "8px" }}>
          {[`🟢 ${liveApps} apps live`, `💰 $${monthlyCost.toFixed(2)}/mo confirmed`, verifySubs > 0 ? `⚠️ ${verifySubs} to verify` : null, `✅ ${postedCount} posted · 📅 ${scheduledCount} scheduled`].filter(Boolean).map(s => (
            <span key={s} style={{ background: "#2A2010", color: "#C8B88A", padding: "4px 10px", borderRadius: "8px", fontSize: "11px", fontWeight: 700 }}>{s}</span>
          ))}
        </div>
      </div>

      {/* Tab Bar */}
      <div style={{ background: "#2A2010", display: "flex", padding: "0 16px", gap: "4px" }}>
        {[["empire", "🏗️ My Empire"], ["stack", "💳 My Stack"], ["social", "📣 Social"]].map(([id, label]) => (
          <button key={id} onClick={() => setTab(id)} style={{ padding: "10px 16px", border: "none", background: "none", borderBottom: tab === id ? "2px solid #E8B931" : "2px solid transparent", color: tab === id ? "#E8B931" : "#888", fontWeight: 800, fontSize: "11px", cursor: "pointer", fontFamily: "inherit", textTransform: "uppercase", letterSpacing: "0.8px" }}>{label}</button>
        ))}
      </div>
      ...
      <div style={{ padding: "16px" }}>
          ...
          <>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px", flexWrap: "wrap", gap: "8px" }}>
              <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                {UNIVERSES_EMP.map(u => <button key={u} onClick={() => setAppFilter(u)} style={{ padding: "4px 12px", borderRadius: "16px", border: "1.5px solid", borderColor: appFilter === u ? "#E8B931" : "#ddd", background: appFilter === u ? "#E8B93118" : "#fff", color: appFilter === u ? "#8B6D3F" : "#888", fontWeight: 700, fontSize: "11px", cursor: "pointer", fontFamily: "inherit" }}>{u}</button>)}
              </div>
              <button onClick={() => setShowAddApp(true)} style={{ background: "#E8B931", border: "none", color: "#1A1A1A", padding: "7px 16px", borderRadius: "8px", fontWeight: 800, fontSize: "11px", cursor: "pointer", fontFamily: "inherit" }}>+ Add App</button>
            </div>
            {apps.length === 0 && <div style={{ textAlign: "center", padding: "40px", color: "#aaa", fontSize: "13px" }}>No apps yet. Add your first one.</div>}
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "12px" }}>
              {filteredApps.map(app => {
                const s = STATUS_COLORS_EMP[app.status] || STATUS_COLORS_EMP.planning;
                return (
                  <div key={app.id} style={{ background: "#fff", borderRadius: "12px", padding: "16px", borderLeft: `4px solid ${app.color || "#c9a84c"}`, boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px" }}>
                      <span style={{ fontWeight: 800, fontSize: "13px", color: "#1A1A1A" }}>{app.name}</span>
                      <span style={{ fontSize: "10px", background: s.bg, color: s.text, padding: "2px 7px", borderRadius: "6px", fontWeight: 700 }}>{s.label}</span>
                    </div>
                    {app.description && <div style={{ fontSize: "11px", color: "#666", marginBottom: "8px", lineHeight: 1.4 }}>{app.description}</div>}
                    <div style={{ fontSize: "10px", color: "#aaa", marginBottom: "8px" }}>{app.stack}{app.universe ? ` · ${app.universe}` : ""}</div>
                    {app.url && app.url !== "—" && <a href={`https://${app.url}`} target="_blank" rel="noreferrer" style={{ fontSize: "10px", color: "#4A6FA5", textDecoration: "none" }}>🔗 {app.url}</a>}
                    <div style={{ display: "flex", gap: "6px", marginTop: "10px" }}>
                      <button onClick={() => setEditingApp({...app})} style={{ fontSize: "10px", background: "#F5F0E8", border: "none", padding: "4px 10px", borderRadius: "6px", cursor: "pointer", fontWeight: 700, color: "#888" }}>Edit</button>
                      <button onClick={() => removeApp(app.id)} style={{ fontSize: "10px", background: "#fff0f0", border: "none", padding: "4px 10px", borderRadius: "6px", cursor: "pointer", fontWeight: 700, color: "#c0392b" }}>Delete</button>
                    </div>
                  </div>
                );
              })}
        ...
        {tab === "stack" && (
          <>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px", flexWrap: "wrap", gap: "8px" }}>
              <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                {CATEGORIES_EMP.map(c => <button key={c} onClick={() => setSubFilter(c)} style={{ padding: "4px 12px", borderRadius: "16px", border: "1.5px solid", borderColor: subFilter === c ? "#E8B931" : "#ddd", background: subFilter === c ? "#E8B93118" : "#fff", color: subFilter === c ? "#8B6D3F" : "#888", fontWeight: 700, fontSize: "11px", cursor: "pointer", fontFamily: "inherit" }}>{c}</button>)}
              </div>
              <button onClick={() => setShowAddSub(true)} style={{ background: "#E8B931", border: "none", color: "#1A1A1A", padding: "7px 16px", borderRadius: "8px", fontWeight: 800, fontSize: "11px", cursor: "pointer", fontFamily: "inherit" }}>+ Add Service</button>
            </div>
            {subs.length === 0 && <div style={{ textAlign: "center", padding: "40px", color: "#aaa", fontSize: "13px" }}>No subscriptions yet. Add what you're paying for.</div>}
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: "10px" }}>
              {filteredSubs.map(sub => {
                const s = SUB_STATUS_EMP[sub.status] || SUB_STATUS_EMP.active;
                return (
                  <div key={sub.id} style={{ background: "#fff", borderRadius: "10px", padding: "14px", boxShadow: "0 2px 6px rgba(0,0,0,0.05)" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                      <span style={{ fontWeight: 800, fontSize: "13px" }}>{sub.name}</span>
                      <span style={{ fontSize: "10px", background: s.bg, color: s.text, padding: "2px 7px", borderRadius: "6px", fontWeight: 700 }}>{s.label}</span>
                    </div>
                    <div style={{ fontSize: "12px", color: "#666", marginBottom: "4px" }}>{sub.cost > 0 ? `$${sub.cost}/${sub.billing === "monthly" ? "mo" : sub.billing}` : "Free"} · {sub.category}</div>
                    {sub.notes && <div style={{ fontSize: "11px", color: "#aaa", marginBottom: "8px" }}>{sub.notes}</div>}
                    <div style={{ display: "flex", gap: "6px" }}>
                      <button onClick={() => setEditingSub({...sub})} style={{ fontSize: "10px", background: "#F5F0E8", border: "none", padding: "4px 10px", borderRadius: "6px", cursor: "pointer", fontWeight: 700, color: "#888" }}>Edit</button>
                      <button onClick={() => removeSub(sub.id)} style={{ fontSize: "10px", background: "#fff0f0", border: "none", padding: "4px 10px", borderRadius: "6px", cursor: "pointer", fontWeight: 700, color: "#c0392b" }}>Delete</button>
                    </div>
                  </div>
                );
              })}
          ...
          <>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px", flexWrap: "wrap", gap: "8px" }}>
              <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", alignItems: "center" }}>
                {["All", ...EMP_PLATFORMS].map(p => (
                  <button key={p} onClick={() => setCalFilter(p)} style={{ padding: "4px 12px", borderRadius: "16px", border: "1.5px solid", borderColor: calFilter === p ? "#C7366B" : "#ddd", background: calFilter === p ? "#C7366B18" : "#fff", color: calFilter === p ? "#C7366B" : "#888", fontWeight: 700, fontSize: "11px", cursor: "pointer", fontFamily: "inherit" }}>
                    {p !== "All" && EMP_PLATFORM_COLORS[p]?.emoji + " "}{p}
                  </button>
                ))}
              </div>
              <button onClick={() => setShowAddPost(true)} style={{ background: "#E8B931", border: "none", color: "#1A1A1A", padding: "7px 16px", borderRadius: "8px", fontWeight: 800, fontSize: "11px", cursor: "pointer", fontFamily: "inherit" }}>+ New Post</button>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: "8px" }}>
              {EMP_DAYS.map(day => {
                const dayPosts = filteredPosts.filter(p => p.day === day);
                return (
                  <div key={day} style={{ background: "#FAFAF8", borderRadius: "10px", border: "1.5px solid #EAE6DC", minHeight: "140px", overflow: "hidden" }}>
                    <div style={{ background: "#2D2520", padding: "5px 8px", fontWeight: 800, fontSize: "9px", color: "#E8B931", textTransform: "uppercase", letterSpacing: "1px" }}>{day}</div>
                    <div style={{ padding: "6px", display: "flex", flexDirection: "column", gap: "5px" }}>
                      {dayPosts.map(post => {
                        const pc = EMP_PLATFORM_COLORS[post.platform] || { bg: "#888", emoji: "📢" };
                        const ps = EMP_POST_STATUS[post.status] || EMP_POST_STATUS.idea;
                        return (
                          <div key={post.id} onClick={() => setEditingPost({...post})} style={{ cursor: "pointer", background: "#fff", borderRadius: "6px", padding: "6px 7px", borderLeft: `3px solid ${pc.bg}` }}>
                            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "3px" }}>
                              <span style={{ fontSize: "9px", fontWeight: 800, color: pc.bg }}>{pc.emoji} {post.platform}</span>
                              <span style={{ fontSize: "7px", background: ps.bg, color: ps.text, padding: "1px 4px", borderRadius: "3px", fontWeight: 700 }}>{ps.label}</span>
                            </div>
                            <div style={{ fontSize: "9px", color: "#555", lineHeight: 1.3, display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{post.content}</div>
                          </div>
                        );
                      })}
                      <button onClick={() => { setNewPost(p => ({...p, day})); setShowAddPost(true); }} style={{ background: "none", border: "1px dashed #ddd", borderRadius: "5px", padding: "3px", fontSize: "9px", color: "#bbb", cursor: "pointer" }}>+ add</button>
                    </div>
                  </div>
                );
              })}
      ...
      {(showAddApp || editingApp) && (() => {
        ...
        const form = isEdit ? editingApp : newApp;
        const setForm = isEdit ? setEditingApp : setNewApp;
        const onSave = isEdit ? () => { saveApp(editingApp); setEditingApp(null); } : () => { saveApp(newApp); setNewApp({ name: "", url: "", stack: "", status: "planning", universe: "", description: "", color: "#c9a84c" }); setShowAddApp(false); };
        const onClose = isEdit ? () => setEditingApp(null) : () => setShowAddApp(false);
        return (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 10000, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
            <div style={{ background: "#fff", borderRadius: "16px", width: "100%", maxWidth: "480px", padding: "24px", boxShadow: "0 20px 60px rgba(0,0,0,0.25)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "16px" }}><h3 style={{ margin: 0, fontWeight: 800 }}>{isEdit ? "Edit App" : "Add App"}</h3><button onClick={onClose} style={{ background: "none", border: "none", fontSize: "18px", cursor: "pointer", color: "#aaa" }}>×</button></div>
              {[["Name", "name", "text"], ["URL", "url", "text"], ["Stack", "stack", "text"], ["Universe", "universe", "text"]].map(([l, k, t]) => (
                <div key={k} style={{ marginBottom: "10px" }}><label style={labelS}>{l}</label><input type={t} style={inputS} value={form[k] || ""} onChange={e => setForm(p => ({...p, [k]: e.target.value}))} /></div>
              ))}
              <div style={{ marginBottom: "10px" }}><label style={labelS}>Status</label><select style={inputS} value={form.status} onChange={e => setForm(p => ({...p, status: e.target.value}))}>{Object.entries(STATUS_COLORS_EMP).map(([k,v]) => <option key={k} value={k}>{v.label}</option>)}</select></div>
              <div style={{ marginBottom: "16px" }}><label style={labelS}>Description</label><textarea style={{...inputS, resize: "vertical"}} rows={3} value={form.description || ""} onChange={e => setForm(p => ({...p, description: e.target.value}))} /></div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                {isEdit && <button onClick={() => { removeApp(editingApp.id); setEditingApp(null); }} style={{ background: "none", border: "1.5px solid #f8d7da", color: "#c0392b", padding: "8px 16px", borderRadius: "8px", cursor: "pointer", fontWeight: 700, fontSize: "12px" }}>Delete</button>}
                <div style={{ display: "flex", gap: "8px", marginLeft: "auto" }}>
                  <button onClick={onClose} style={{ background: "none", border: "1.5px solid #ddd", padding: "8px 16px", borderRadius: "8px", cursor: "pointer", fontWeight: 700, fontSize: "12px", color: "#888" }}>Cancel</button>
                  <button onClick={onSave} style={{ background: "#1A1A1A", border: "none", color: "#E8B931", padding: "8px 20px", borderRadius: "8px", cursor: "pointer", fontWeight: 800, fontSize: "12px" }}>Save</button>
                </div>
              </div>
            </div>
          </div>
      ...
      {(showAddSub || editingSub) && (() => {
        ...
        const form = isEdit ? editingSub : newSub;
        const setForm = isEdit ? setEditingSub : setNewSub;
        const onSave = isEdit ? () => { saveSub(editingSub); setEditingSub(null); } : () => { saveSub(newSub); setNewSub({ name: "", category: "", cost: 0, billing: "monthly", status: "active", notes: "", url: "" }); setShowAddSub(false); };
        const onClose = isEdit ? () => setEditingSub(null) : () => setShowAddSub(false);
        return (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 10000, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
            <div style={{ background: "#fff", borderRadius: "16px", width: "100%", maxWidth: "480px", padding: "24px", boxShadow: "0 20px 60px rgba(0,0,0,0.25)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "16px" }}><h3 style={{ margin: 0, fontWeight: 800 }}>{isEdit ? "Edit Service" : "Add Service"}</h3><button onClick={onClose} style={{ background: "none", border: "none", fontSize: "18px", cursor: "pointer", color: "#aaa" }}>×</button></div>
              {[["Name", "name"], ["Category", "category"], ["URL", "url"]].map(([l, k]) => (
                <div key={k} style={{ marginBottom: "10px" }}><label style={labelS}>{l}</label><input style={inputS} value={form[k] || ""} onChange={e => setForm(p => ({...p, [k]: e.target.value}))} /></div>
              ))}
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "10px" }}>
                <div><label style={labelS}>Cost</label><input type="number" style={inputS} value={form.cost || 0} onChange={e => setForm(p => ({...p, cost: parseFloat(e.target.value) || 0}))} /></div>
                <div><label style={labelS}>Billing</label><select style={inputS} value={form.billing} onChange={e => setForm(p => ({...p, billing: e.target.value}))}>{["monthly","annual","per-transaction","royalty-share","unknown"].map(b => <option key={b}>{b}</option>)}</select></div>
              </div>
              <div style={{ marginBottom: "10px" }}><label style={labelS}>Status</label><select style={inputS} value={form.status} onChange={e => setForm(p => ({...p, status: e.target.value}))}>{Object.entries(SUB_STATUS_EMP).map(([k,v]) => <option key={k} value={k}>{v.label}</option>)}</select></div>
              <div style={{ marginBottom: "16px" }}><label style={labelS}>Notes</label><textarea style={{...inputS, resize: "vertical"}} rows={2} value={form.notes || ""} onChange={e => setForm(p => ({...p, notes: e.target.value}))} /></div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                {isEdit && <button onClick={() => { removeSub(editingSub.id); setEditingSub(null); }} style={{ background: "none", border: "1.5px solid #f8d7da", color: "#c0392b", padding: "8px 16px", borderRadius: "8px", cursor: "pointer", fontWeight: 700, fontSize: "12px" }}>Delete</button>}
                <div style={{ display: "flex", gap: "8px", marginLeft: "auto" }}>
                  <button onClick={onClose} style={{ background: "none", border: "1.5px solid #ddd", padding: "8px 16px", borderRadius: "8px", cursor: "pointer", fontWeight: 700, fontSize: "12px", color: "#888" }}>Cancel</button>
                  <button onClick={onSave} style={{ background: "#1A1A1A", border: "none", color: "#E8B931", padding: "8px 20px", borderRadius: "8px", cursor: "pointer", fontWeight: 800, fontSize: "12px" }}>Save</button>
                </div>
      ...
      {(showAddPost || editingPost) && (() => {
        ...
        const form = isEdit ? editingPost : newPost;
        const setForm = isEdit ? setEditingPost : setNewPost;
        const onSave = isEdit ? () => { savePost(editingPost); setEditingPost(null); } : () => { savePost(newPost); setNewPost({ day: "Monday", platform: "Facebook", content: "", status: "idea", link: "" }); setShowAddPost(false); };
        const onClose = isEdit ? () => setEditingPost(null) : () => setShowAddPost(false);
        return (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 10000, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
            <div style={{ background: "#fff", borderRadius: "16px", width: "100%", maxWidth: "480px", padding: "24px", boxShadow: "0 20px 60px rgba(0,0,0,0.25)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "16px" }}><h3 style={{ margin: 0, fontWeight: 800 }}>{isEdit ? "Edit Post" : "New Post"}</h3><button onClick={onClose} style={{ background: "none", border: "none", fontSize: "18px", cursor: "pointer", color: "#aaa" }}>×</button></div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "10px" }}>
                <div><label style={labelS}>Day</label><select style={inputS} value={form.day} onChange={e => setForm(p => ({...p, day: e.target.value}))}>{EMP_DAYS.map(d => <option key={d}>{d}</option>)}</select></div>
                <div><label style={labelS}>Platform</label><select style={inputS} value={form.platform} onChange={e => setForm(p => ({...p, platform: e.target.value}))}>{EMP_PLATFORMS.map(p => <option key={p}>{p}</option>)}</select></div>
              </div>
              <div style={{ marginBottom: "10px" }}><label style={labelS}>Status</label><select style={inputS} value={form.status} onChange={e => setForm(p => ({...p, status: e.target.value}))}>{Object.entries(EMP_POST_STATUS).map(([k,v]) => <option key={k} value={k}>{v.label}</option>)}</select></div>
              <div style={{ marginBottom: "10px" }}><label style={labelS}>Content</label><textarea style={{...inputS, resize: "vertical"}} rows={5} value={form.content} onChange={e => setForm(p => ({...p, content: e.target.value}))} placeholder="What are you saying this week?" /></div>
              <div style={{ marginBottom: "16px" }}><label style={labelS}>Link (optional)</label><input style={inputS} value={form.link || ""} onChange={e => setForm(p => ({...p, link: e.target.value}))} placeholder="https://..." /></div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                {isEdit && <button onClick={() => { removePost(editingPost.id); setEditingPost(null); }} style={{ background: "none", border: "1.5px solid #f8d7da", color: "#c0392b", padding: "8px 16px", borderRadius: "8px", cursor: "pointer", fontWeight: 700, fontSize: "12px" }}>Delete</button>}
                <div style={{ display: "flex", gap: "8px", marginLeft: "auto" }}>
                  <button onClick={onClose} style={{ background: "none", border: "1.5px solid #ddd", padding: "8px 16px", borderRadius: "8px", cursor: "pointer", fontWeight: 700, fontSize: "12px", color: "#888" }}>Cancel</button>
                  <button onClick={onSave} style={{ background: "#1A1A1A", border: "none", color: "#E8B931", padding: "8px 20px", borderRadius: "8px", cursor: "pointer", fontWeight: 800, fontSize: "12px" }}>Save</button>
                </div>
      ...
      {showEmpire && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 9998, display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ flex: 1, overflowY: "auto" }}>
            <EmpireCommandCenter userId={user.id} />
          </div>
        </div>
      )}
      {showTabManager && (
